...
test_that("getData works with urls without header", {
  ...
  expect_equal(getData("http://httpbin.org/status/404"),"check URL and try again")
})
...
test_that("getData works with urls with header", {
  ...
  expect_equal(getData("http://httpbin.org/status/404",headers = '{"Key":"Value"}'),"check URL and try again")
  ...
  expect_equal(getData("http://httpbin.org/status/404",headers = list(Key="Value")),"check URL and try again")
})
